...
const State = () => {
  ...
  const fetchAllStates = async () => {
    try {
      ...
      const [activeRes, inactiveRes] = await Promise.all([
        ...
        axios.get(`${signupApi}state/inactive`),
      ]);
      setStates(activeRes.data.result || []);
      setInactiveStates(inactiveRes.data.result || []);
    } catch (error) {
      console.error(error);
  ...
  const filteredData = (activeTab === "active" ? states : inactiveStates)
    .filter((item) => item.stateName.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => sortOrder === "asc" ? a.stateName.localeCompare(b.stateName) : b.stateName.localeCompare(a.stateName));
